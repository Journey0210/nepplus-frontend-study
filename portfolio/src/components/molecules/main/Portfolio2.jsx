...
const Portfolio2 = ()=>{
    ...
    <Right>
        ...
        <b>구현 기능</b>:
        <ul>       
        <li><b>로그인 페이지에서 일정 시간이 지나면 이미지가 자동으로 변화하는 carousel을 만들었습니다.</b><br/> 
         → setInterval 함수를 사용해 4초 간격으로 이미지가 index에 따라 순차적으로 바뀌도록 했고,  
        이를 useEffect로 감싸 컴포넌트가 재렌더링 될 때 실행시킬 cleanup함수를 추가했습니다.<br/> 
        </li>
        <li><b>nav 버튼을 통해 다른 페이지로 이동할 때에도, 상단에 nav가 고정되어 있도록 했습니다.</b><br/> 
        →  nav 컴포넌트에 중첩라우팅을 구성하고, react router dom의 Outlet을 사용했습니다.<br/> 
        </li>
        <li><b>nav에서 새 게시물 추가 버튼<NewPost/> 클릭시, modal창이 열린 상태에서는 스크롤 바가 움직이지 않도록 했습니다.</b><br/> 
        →  모달창이 열렸을 때 화면이 스크롤이 되지 않도록 body 전체에  overflow = "hidden"을 주었고, 
        modal창이 사라지면 위 기능을 제거되도록 했습니다.<br/> 
        </li>
        <li><b>새 게시물을 추가하는 모달창에서, 내 컴퓨터에 있는 이미지 파일을 불러와 화면에서 미리보기를 할 수 있도록 했습니다.</b><br/> 
